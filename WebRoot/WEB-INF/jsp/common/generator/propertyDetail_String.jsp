					...
					<tr>
					 ...
					 <tr>
					  ...
					  <tr>
						<th>最大长度</th>
						<td>
						<input type="text" name="maxLength" value="${genEntityProperty.maxLength}" class="easyui-numberbox easyui-validatebox"  data-options="required:true"/>
						</td>
						<th>最小长度 </th>
						<td>
						<input type="text"  name="minLength" value='${genEntityProperty.minLength eq null?0:genEntityProperty.minLength }' class="easyui-numberbox easyui-validatebox"  data-options="required:true"/>
						</td>
					 </tr>
						<tr>
						<th>编辑类型</th>
						<td>
							<select name="editType" class="easyui-combobox" style="width:170px;"  data-options="required:true">
								...
								<option value="textarea" <c:if test="${genEntityProperty.editType eq 'textarea' }">selected="selected"</c:if>>textarea</option>
								<option value="password" <c:if test="${genEntityProperty.editType eq 'password' }">selected="selected"</c:if>>password</option>
								...
								<option value="checkbox" <c:if test="${genEntityProperty.editType eq 'checkbox' }">selected="selected"</c:if>>checkbox</option>
						...
						<th >校验类型<font style="color:red;">(文本)</font></th>
						<td>
						 <select id="validateType" name="validateType" class="easyui-combobox" style="width:170px;"  data-options="required:true">
								<option value="">无</option>
							    <option value="email" <c:if test="${genEntityProperty.validateType eq 'email' }">selected="selected"</c:if>>email</option>
								<option value="url" <c:if test="${genEntityProperty.validateType eq 'url' }">selected="selected"</c:if>>url</option>
								<option value="phone" <c:if test="${genEntityProperty.validateType eq 'phone' }">selected="selected"</c:if>>phone</option>
								<option value="mobile" <c:if test="${genEntityProperty.validateType eq 'mobile' }">selected="selected"</c:if>>mobile</option>
								<option value="ZipCode" <c:if test="${genEntityProperty.validateType eq 'ZipCode' }">selected="selected"</c:if>>ZipCode</option>
								<option value="ip" <c:if test="${genEntityProperty.validateType eq 'ip' }">selected="selected"</c:if>>ip</option>
								<option value="idcard" <c:if test="${genEntityProperty.validateType eq 'idcard' }">selected="selected"</c:if>>idcard</option>
								<option value="intOrFloat" <c:if test="${genEntityProperty.validateType eq 'intOrFloat' }">selected="selected"</c:if>>intOrFloat</option>
								<option value="account" <c:if test="${genEntityProperty.validateType eq 'account' }">selected="selected"</c:if>>account</option>
							</select>
						</td>
					 </tr>
					 <tr>
